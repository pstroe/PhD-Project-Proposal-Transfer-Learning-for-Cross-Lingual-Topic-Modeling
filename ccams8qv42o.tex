...
The aim of transfer learning is to transfer knowledge from an already trained model to a different domain or a different task. One's ability to steer a car, for example, helps in learning to drive a motorcycle. Transfer learning for topic modeling is still a low researched field. In topic modeling, there have been some approaches towards making it more robust when it comes to short text with fast changing topics. \citet{kang2013} incorporate labels from much bigger and data and use it as informative prior in hierarchical LDA. 
