...
\paragraph{Topic modeling - working package 1} The similarities of the topic models when applied to newspaper texts suggest a transfer learning approach, which would try to map topics from one newspaper to another. Labeled newspaper pages or articles can help to produce more stable topic models, using labels as priors as in \citet{kang2013}. Since the amount of labeled data is limited, my first aim is to propagate these labels back in time in order to arrive at a fully labeled data set, at least at the level of an individual newspaper. This allows for topic modeling within the different categories, thus leading to  The next step would be to see how well these labels are transferable to other newspapers of the same language. 
