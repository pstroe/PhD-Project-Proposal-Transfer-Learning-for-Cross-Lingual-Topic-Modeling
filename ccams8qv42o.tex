...
\citet{EisensteinAX11} present an alternative to topic modeling using Dirichlets. They highlight three shortcomings of LDA: inference cost, overparametrisation, and lack of sparsity. The key idea is to use Sparse Additive Generative (SAGE) models, where the topics are replaced with deviations from a background word distribution. As such, topics are charachterised by a smaller number of words than is the case for LDA. Moreover, it facilitates the incorporation of other facets, like the time a document was written, sentiment, or syntax, making SAGE a multi-faceted model. Last but not least, SAGE turns out to be more efficient than LDA.
