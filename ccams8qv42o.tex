...
There are other techniques than LDA for topic modeling. \citet{EisensteinAX11} present an alternative by using Sarse Additive Genertive (SAGE) model. They highlight three shortcomings of LDA: inference cost, overparametrisation, and lack of sparsity. The key idea is to use  where the topics are replaced with deviations from a background word distribution. As such, topics are charachterised by a smaller number of words than is the case for LDA. Moreover, it facilitates the incorporation of other facets, like the time a document was written, sentiment, or syntax, making SAGE a multi-faceted model. Last but not least, SAGE turns out to be more efficient than LDA.
