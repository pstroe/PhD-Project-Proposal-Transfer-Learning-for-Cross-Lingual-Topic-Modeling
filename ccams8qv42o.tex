...
\{Word embeddings for topic modeling} Many techniques in natural language processing (NLP) profited from the emergence of word embeddings . Since we exptect the words which describe a topic to be semantically related, ind word embeddings into topic models can be benefitial. \citet{dasgaussian2015} successfully incorporate word embeddings into Gaussian LDA and report that their technique outperforms standard LDA especially when dealing with out-of-vocabulary words.
